...
#'
#' @param mlm_model it requires an mlm_model, generated by lme4. If this is a null, the plot display the unconditioned random effect. If the model is not a null model, then the plot display the expected random effects given the model.
#'
...
#'
#'
caterpillar_plot <- function(mlm_model){
  ## required libraries
  require(ggplot2)
  require(lme4)
  require(dplyr)
  ...
  rand_intercept <- tibble::as_tibble(lme4::ranef(mlm_model)) %>%
    ...
    mutate(u_j = condval) %>%
    mutate(ll = condval-condsd*1.96) %>%
    mutate(ul = condval+condsd*1.96) %>%
